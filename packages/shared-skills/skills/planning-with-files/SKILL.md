...
hooks:
  ...
  Stop:
    - hooks:
        - type: command
          command: "SD=\"{{skill_dir}}/scripts\"; powershell.exe -NoProfile -ExecutionPolicy RemoteSigned -File \"$SD/check-complete.ps1\" 2>/dev/null || sh \"$SD/check-complete.sh\""
metadata:
  version: "2.37.0"
...
```powershell
# Windows PowerShell
python "{{skill_dir}}/scripts/session-catchup.py" (Get-Location)
```
...
|----------|-----------------|
| Skill directory (`{{skill_dir}}/`) | Templates, scripts, reference docs |
